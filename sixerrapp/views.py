...
braintree.Configuration.configure(braintree.Environment.Sandbox,
                                    merchant_id="d2ss24h55df2z7zr",
                                    public_key="t2yz7qtysqvcdyy2",
                                    private_key="997442b0f4dabb5a9ae90bec6a512eae")
...
def gig_detail(request, id):
    if request.method == "POST" and \
        ...
        Purchase.objects.filter(gig_id, buyer=request.user).count() > 0 and \
...
def my_offers(request):
    ...
    return render(request, 'my_offers.html', {"purchases": purchases})
